extends Node3D
# Quad carousel — runtime-loads each UniRig-rigged quad's animated GLB (Blender-keyed
# walk), plays it looping, arranges in a row on a slow turntable. Runtime GLTFDocument
# so no editor import; animation + texture come through.
const CHARS := ["hell_hound", "bone_hound", "grave_boar", "dire_rat"]
const TARGET_H := 1.5
const SPACING := 2.1
var _pivots: Array = []
var _shot := false
...
func _ready() -> void:
	...
	for i in range(n):
		...
		if doc.append_from_file("res://quads/%s_walk.glb" % CHARS[i], st) != OK:
			push_warning("load failed %s" % CHARS[i]); continue
		...
		if ap:
			...
			if l.size() > 0:
				var a := ap.get_animation(l[0])
				if a: a.loop_mode = Animation.LOOP_LINEAR
				ap.play(l[0])
		var lab := Label3D.new()
		lab.text = CHARS[i]
		lab.font_size = 40; lab.pixel_size = 0.004
		lab.billboard = BaseMaterial3D.BILLBOARD_ENABLED
		lab.position = Vector3(0, -0.28, 0); lab.modulate = Color(0.95, 0.88, 0.72)
		pivot.add_child(lab)
	var ground := MeshInstance3D.new()
	var pm := PlaneMesh.new(); pm.size = Vector2(maxf(n*SPACING+4, 8), 8); ground.mesh = pm
...
func _process(delta: float) -> void:
	...
	if not _shot and _elapsed > 2.5:
		...
		get_viewport().get_texture().get_image().save_png("C:/Users/scher/.claude/jobs/6d258183/tmp/quad_carousel.png")
