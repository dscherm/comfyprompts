...
```json
{
  ...
  "passes": true,
  "resolution": "DONE (in-place bake of a forward walk). The Mixamo walking.fbx (forward walk, 1.66u) was Humanoid-retargeted onto the knight in Unity (soapbox-unity/CoplayScripts/BakeKnightWalkRM.cs) then baked IN PLACE (net drift stripped) -> revenant_knight_walk_rm.fbx: a brisk, UPRIGHT, natural-cadence walk whose foot-plant speed is 0.74 m/s at 1.0x. player.gd plays it at native 1.0x with WALK_SPEED 0.74 (matched -> no skate). An earlier root-motion variant (folding the forward hips translation in + AnimationTree root_motion_track) was reverted: it caused a forward LEAN (hip lunging ahead in the pose) and DISTORTED the run (the global root_motion_track extraction pulls motion out of the in-place run/idle clips too). The in-place + measured-speed approach is simpler and fixes both. Run keeps its measured 1.4/0.96x match."
}
```
