...
- [~] **D0.8** List **`dissonant_style`** (DissonantDreams retro-futurist) LoRA **FREE** on CivitAI. *(TRAINED + evaled 2026-06-30 — Phase DS in plan.md; PASS, distinctive/transferable, winner ckpt 1500 @ 1.0. Deployed to ComfyUI loras + sidecar. Listing copy ready: `scripts/train_lora/eval/dissonant_style_listing.md`; 5 hero samples in `eval/dissonant_style_assets/`. Needs account upload — FREE only, Flux-dev license; cross-promote the DissonantDreams game. This is the distinctive-own-art play that replaced scrapped stylized_game.)*
- [~] **D0.9** List **`soapbox_style`** (gritty cartoon kart-mascot) LoRA **FREE** on CivitAI. *(TRAINED + evaled 2026-06-30 — Phase SX in plan.md; PASS, distinctive/transferable, winner ckpt 1500 @ 0.8. Augment-first pipeline turned 64px game sprites into a trainable style; transfers cleanly to NEW mascots — robot/frog/wizard/shark. Deployed to ComfyUI loras/style + sidecar `.txt`; 8-sample card `eval/soapbox_style_card.png`. Listing copy ready: `scripts/train_lora/eval/soapbox_style_listing.md`; gallery samples in `eval/soapbox_style_assets/`. Needs account upload — FREE only, Flux-dev license; cross-promote the Soapbox Sabotage game.)*
- [~] **D0.10** List **`soapbox_racers`** (gritty comic-book character style) LoRA **FREE** on CivitAI. *(REDO of the soapbox character LoRA, 2026-07-01 — the user wanted gritty illustrative/comic (Mad Max + Speed Racer + R. Crumb + Otomo + Mad Magazine), NOT the cartoonish v1. Trained on the user's own game art (character_variations, gritty ink) + low-denoise augment, 132 imgs, clean per-character captions. Shipped step 500 @ strength 1.1 — PASS on all 9 characters (player/bones/crank/grit/pip/punk_king/rust/smog/sparks), standing, bold ink. Deployed to ComfyUI loras/style + sidecar; 9-char roster `eval/soapbox_racers_assets/_roster9.png`; eval `soapbox_racers_grid.md`; listing `soapbox_racers_listing.md`. Needs account upload — FREE only, Flux-dev license.)*
- [~] **D0.11** List **`soapbox_world`** (post-apocalyptic wasteland scenes/vehicles) LoRA **FREE** on CivitAI. *(Companion to soapbox_racers — the 2nd of the two-LoRA split. Trained 2026-07-01 on 38 game cutscenes + 7 karts; cut early at step 1000 @ strength 1.0 — PASS, bold comic wasteland race scenes (desert sunsets, ruined cities, junkyards, canyons). Deployed to ComfyUI loras/style + sidecar; 6-scene showcase `eval/soapbox_world_assets/_scenes.png`; eval `soapbox_world_grid.md`; listing `soapbox_world_listing.md`. Needs account upload — FREE only, Flux-dev license.)*
- [ ] **SK0** List **Soapbox Kart Kit** (26 procedural pieces + 5 Hunyuan3D mascot racers) on **itch.io + Fab + Unity**. *(Built 2026-07-01, committed 4994cb8. README + LISTING ready in `products/soapbox_kart_kit_v1/`; hero + catalog + turntable gallery done. ~$6.99-9.99. Needs account upload + zip of models_glb/obj/fbx + mascots/. Procedural pieces + AI-disclosed mascots; cross-promote soapbox_racers LoRA + the game.)*
...
### Stream L — DissonantCity retro-futurist 3D kit (NEW — DissonantDreams aesthetic version of Stream K)
*Same proven procedural-Blender / flat-shaded low-poly approach as Stream K (the GrimForge village), re-skinned to the **DissonantDreams retro-futurist palette** (hot pink / cyan / black / cream + neon). Generator: `tools/asset_generators/dissonant_city/`. Headless Blender (blender-mcp down → documented fallback). Pairs with the `dissonant_style` LoRA — a matching 2D+3D DissonantDreams line.*
- [x] **L0.1** PoC hero — `hero_poc.py`: procedural retro-futurist block (tapered towers, chrome dome, neon-window slab, skybridge, hover-cars, neon grid floor) in the DissonantDreams palette, dusk mood, ortho render. Confirms the look translates to low-poly. → `scratchpad/dissonant_city_hero.png`.
- [x] **L0.2** Locked the look after refinement (user approved direction): added a **numpy bloom post-step** (`bloom.py` — EEVEE Next dropped legacy bloom; mask hot pixels → gaussian blur → screen-blend) so neon GLOWS; brightened lights + world + emission; fixed the over-bloomed dome (dropped its emission → reads as solid chrome). Result = cohesive DissonantDreams neon low-poly block (cyan/purple towers, chrome dome, pink neon-window slabs, skybridge, neon grid floor). Hero: `scratchpad/dissonant_city_hero.png`.
- [x] **L0.3** Built the piece library — **26 modular pieces** (`build_pieces.py`): 7 buildings (tower_tall ×2, tower_short, tower_cyl, dome_building, arcology, ziggurat), 2 neon slab-shops, skybridge + support, 4 ground/road tiles (straight/corner/junction/plaza), + props (streetlight, billboard, neon_arch, 2 hover-cars, antenna, holo_pylon, fountain_pad, crystals, palm_retro, barrier). DissonantDreams palette, flat-shaded low-poly, grid-modular (2u). Kit hero assembled + rendered (`hero_kit.py` + bloom) → coherent retro-futurist city. `scratchpad/dissonant_city_kit_hero.png`.
- [x] **L0.4** Exported **GLB + OBJ + FBX** per piece (26 each) to `products/dissonant_city_v1/models_{glb,obj,fbx}/` (build_pieces.py exports all 3 formats; mesh binaries gitignored like the village kit).
- [x] **L0.5** Hero render + catalog + README + LISTING in `products/dissonant_city_v1/` *(2026-06-30 — `hero.png` assembled dusk-neon city + `catalog.png` all-29 contact sheet, both bloomed via `bloom.py`; dark-navy bodies + glowing pink/cyan neon read correctly. `catalog_render.py` added. README/LISTING ready — title "DissonantCity — Retro-Futurist Neon Low-Poly City Kit (29 Pieces)", $5.99–7.99, cyberpunk/synthwave tags. Human-gated upload tracked at L0.7.)*
- [ ] **L0.6** Godot validation — import GLBs, reconstruct a city scene, materials intact.
- [ ] **L0.7** List on **itch.io + Fab + Unity** (free base or $4.99 PWYW); AI-disclosure note (procedural Blender, not AI-gen — minimal). Cross-promote with `dissonant_style` + the DissonantDreams game.
