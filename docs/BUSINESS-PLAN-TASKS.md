...
- [~] **L0.2** Lock the look after feedback. *(1st pass good — palette + structure right. Refine: stronger neon GLOW (EEVEE Next dropped legacy bloom → add a compositor glare pass), brighter overall, push saturation toward the high-contrast source art, brighten the chrome dome. Awaiting user steer.)*
