        ...
        **Workflow:**
        ...
        Args:
            ...
            target_filename: Optional target filename (e.g., "hero.png"). If omitted, auto-generated.
                Must match regex: ^[a-z0-9][a-z0-9_-]{0,62}\\.(webp|png|jpg|jpeg)$
            manifest_key: Optional manifest key (required if target_filename omitted).
                Must match regex: ^[a-z0-9][a-z0-9_-]{0,62}$
        ...
        try:
            ...
            if target_filename:
                ...
                if not validate_target_filename(target_filename):
                    return {
                        "error": f"Invalid target_filename: '{target_filename}'. Must match regex: ^[a-z0-9][a-z0-9_-]{{0,62}}\\.(webp|png|jpg|jpeg)$",
                ...
                if not validate_manifest_key(manifest_key):
                    return {
                        "error": f"Invalid manifest_key: '{manifest_key}'. Must match regex: ^[a-z0-9][a-z0-9_-]{{0,62}}$",
            ...
            # Dispatch asset_published webhook event
            if webhook_manager:
                try:
                    webhook_manager.dispatch("asset_published", {
                        "asset_id": asset_id,
                        ...
                        "manifest_key": manifest_key,
                        "web_optimized": web_optimize
                    })
                except Exception as e:
                    logger.warning(f"Failed to dispatch asset_published webhook: {e}")
