    ...
    def _register_workflow_tool(definition: WorkflowToolDefinition):
        def _tool_impl(*args, **kwargs):
            ...
            if definition.workflow_id in ("generate_song", "generate_speech", "generate_sfx", "voice_clone", "video_to_audio"):
                namespace = "audio"
            elif definition.workflow_id in ("generate_video", "generate_video_animatediff", "generate_video_cogvideo", "lip_sync"):
                namespace = "video"
            elif definition.workflow_id in ("generate_3d", "generate_mesh", "image_to_3d"):
                namespace = "3d"
            
            try:
                # Only validate model if the workflow has a model parameter
                if "model" in definition.parameters:
                ...
                if "model" in definition.parameters and ("model" in error_str or "checkpoint" in error_str or "ckpt" in error_str):
    ...
    def regenerate(
        ...
        try:
            ...
            if asset.workflow_id:
                ...
                _audio_workflow_ids = {"generate_song", "generate_speech", "generate_sfx", "voice_clone", "video_to_audio"}
                _video_workflow_ids = {"generate_video", "generate_video_animatediff", "generate_video_cogvideo", "image_to_video", "lip_sync"}
                if asset.workflow_id in _audio_workflow_ids or "audio" in asset.workflow_id.lower() or "song" in asset.workflow_id.lower():
                    output_preferences = ("audio", "audios", "sound", "files")
                elif asset.workflow_id in _video_workflow_ids or "video" in asset.workflow_id.lower():
                    output_preferences = ("videos", "video", "mp4", "mov", "webm")
                elif "image" in asset.workflow_id.lower():
