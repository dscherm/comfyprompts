...
Generate images using Flux architecture.
...
- `cfg` (float): Classifier-free guidance scale. Default: 1.0 (Flux uses low CFG)
- `sampler_name` (str): Sampling method. Default: "euler"
- `scheduler` (str): Scheduler type. Default: "simple"
- `denoise` (float): Denoising strength (0.0-1.0). Default: 1.0
- `negative_prompt` (str): Negative prompt. Default: "" (Flux works best without)
- `return_inline_preview` (bool): Include thumbnail in response. Default: False

**Model:** Uses `flux1-dev-fp8.safetensors` (hardcoded in workflow)
...
**User:** "Create a cyberpunk cityscape, 1024x1024, high quality, 30 steps"

**Agent:** *Calls `generate_image(prompt="cyberpunk cityscape, neon lights, detailed", width=1024, height=1024, steps=30)` → returns asset_id*

**Note:** Flux uses low CFG (default 1.0) and works best without negative prompts.
...
Generate videos using Wan 2.1 architecture.
...
- `height` (int): Video height in pixels. Default: 272
- `frames` (int): Number of frames. Default: 33
...
- `scheduler` (str): Scheduler type. Default: "simple"
- `denoise` (float): Denoising strength (0.0-1.0). Default: 1.0
- `negative_prompt` (str): Negative prompt. Default: "blurry, low quality, distorted"

**Model:** Uses Wan 2.1 14B GGUF (`Wan2.1_14B_VACE-Q4_K_M.gguf`)
...
**Note:** Video generation is VRAM-intensive (~16GB). Use lower resolution/frames if limited.
