...
from tools.upscale import register_upscale_tools
from tools.variations import register_variations_tools
from tools.webhook import register_webhook_tools
from tools.external import register_external_tools
from tools.style_presets import register_style_preset_tools
...
# Generation timeout configuration (in seconds)
# Flux models and other large models can take 3-5+ minutes
GENERATION_TIMEOUT = int(os.getenv("COMFY_MCP_GENERATION_TIMEOUT", "300"))
...
def print_startup_banner():
    ...
    print(f"  Generation timeout: {GENERATION_TIMEOUT} seconds")
    print("=" * 70 + "\n")
...
comfyui_client = ComfyUIClient(base_url=COMFYUI_URL, default_timeout=GENERATION_TIMEOUT)
...
# Initialize webhook manager for event notifications
webhook_manager = WebhookManager()
logger.info("Webhook manager initialized")

# Initialize external app manager for Blender/Unreal integration
try:
    external_app_manager = ExternalAppManager(
        scripts_dir=Path(__file__).parent / "scripts"
    )
    logger.info("External app manager initialized")
except Exception as e:
    logger.warning(f"Failed to initialize external app manager: {e}")
    external_app_manager = None

# Initialize style presets manager
style_presets_manager = StylePresetsManager()
logger.info(f"Style presets manager initialized with {len(style_presets_manager.list_presets())} presets")

# Initialize export presets manager
export_presets_manager = ExportPresetsManager()
logger.info(f"Export presets manager initialized with {len(export_presets_manager.list_presets())} presets")

# Initialize prompt library
prompt_library = PromptLibrary()
logger.info(f"Prompt library initialized with {prompt_library.get_stats()['total_prompts']} prompts")
...
register_workflow_generation_tools(mcp, workflow_manager, comfyui_client, defaults_manager, asset_registry, webhook_manager)
register_regenerate_tool(mcp, comfyui_client, asset_registry, webhook_manager)
register_job_tools(mcp, comfyui_client, asset_registry)

# Register publish tools with webhook support
if publish_manager:
    register_publish_tools(mcp, asset_registry, publish_manager, webhook_manager)
else:
    logger.error("Publish manager not available - publish tools will not be registered")

# Register upscale tools
register_upscale_tools(mcp, comfyui_client, asset_registry, webhook_manager)

# Register variations tools
register_variations_tools(mcp, comfyui_client, defaults_manager, asset_registry, webhook_manager)

# Register webhook management tools
register_webhook_tools(mcp, webhook_manager)

# Register external app integration tools (Blender, Unreal)
if external_app_manager:
    register_external_tools(mcp, external_app_manager, asset_registry)
else:
    logger.warning("External app manager not available - Blender/Unreal tools will not be registered")

# Register style preset tools
register_style_preset_tools(mcp, style_presets_manager)

# Register batch generation tools
register_batch_tools(mcp, workflow_manager, comfyui_client, defaults_manager, asset_registry, style_presets_manager)

# Register export preset tools (social media sizing)
register_export_tools(mcp, export_presets_manager, asset_registry)
