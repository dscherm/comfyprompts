...
# Target filename validation regex: simple filename only, no paths, no dots in name
TARGET_FILENAME_REGEX = re.compile(r'^[a-z0-9][a-z0-9_-]{0,62}\.(webp|png|jpg|jpeg)$')
# Manifest key validation regex: alphanumeric with hyphens/underscores, no dots
MANIFEST_KEY_REGEX = re.compile(r'^[a-z0-9][a-z0-9_-]{0,62}$')
...
class PublishManager:
    ...
    def resolve_source_path(self, subfolder: str, filename: str) -> Path:
        ...
        try:
            ...
            # Path couldn't be resolved - check if it simply doesn't exist
    ...
    def resolve_target_path(self, target_filename: str) -> Path:
        ...
        if not validate_target_filename(target_filename):
            raise ValueError(
                ...
                f"Must match regex: ^[a-z0-9][a-z0-9_-]{{0,62}}\\.(webp|png|jpg|jpeg)$"
            )
    ...
    def update_manifest(self, manifest_key: str, filename: str):
        ...
        if not validate_manifest_key(manifest_key):
            raise ValueError(
                ...
                f"Must match regex: ^[a-z0-9][a-z0-9_-]{{0,62}}$"
            )
