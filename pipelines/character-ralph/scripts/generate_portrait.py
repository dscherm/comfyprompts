"""Generate a character portrait via ComfyUI REST API.
...
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "..", "output", "portrait")
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "portrait.png")

# Parameter substitutions -- keys must match the exact placeholder strings in the workflow JSON
PARAMS: dict[str, str | int | float] = {
    "PARAM_PROMPT": (
        "The Rookie, young male racer, portrait bust shot, facing viewer. "
        "Orange racing jacket with black stripes. Aviator goggles pushed up on forehead. "
        "Short messy brown hair. Confident smirk expression. "
        "Otomo Akira meets R Crumb underground comix style, mechanical precision with "
        "organic weirdness, clean ink with obsessive detail, post-apocalyptic illustration. "
        "Detailed face, sharp focus, clean simple grey background, character concept art, "
        "high quality portrait"
    ),
    "PARAM_NEGATIVE_PROMPT": (
        "blurry, low quality, deformed, ugly, photorealistic, 3D render, smooth digital art, "
        "anime, chibi, cute, Disney, Saturday morning cartoon, watermark, text, signature, "
        "multiple characters"
    ),
    "PARAM_INT_WIDTH": 768,
    "PARAM_INT_HEIGHT": 768,
    "PARAM_INT_SEED": 42,
    "PARAM_INT_STEPS": 25,
    "PARAM_FLOAT_CFG": 1.0,
...
def load_and_substitute(workflow_path: str, params: dict) -> dict:
    ...
    # Replace string placeholders with their typed JSON values.
    # We walk the JSON structure rather than doing naive string replacement
    # so that typed values (int, float) are inserted correctly.
